...
class YouTubeLiveExtractor:
    ...
    def buscar_videos_objetivo(self, query: str = "Claro Republica Dominicana", max_results: int = 10) -> list:
        ...
        try:
            req = self.youtube.search().list(
                part="snippet", q=query, type="video", relevanceLanguage="es", maxResults=max_results, order="relevance"
            )
            resp = req.execute()
            self.quota_used += 100
    ...
    def inferir_categoria(self, texto: str) -> str:
        ...
        if any(
            k in t
            for k in ["fibra", "internet", "wifi", "modem", "ping", "megas", "router", "lento", "subida", "bajada"]
        ):
            ...
            k in t for k in ["servicio", "atencion", "soporte", "call center", "tecnico", "oficina", "reporte", "queja"]
        ):
    ...
    def ejecutar_extraccion_completa(
        self, queries: list = None, max_videos_por_query: int = 10, max_comentarios_por_video: int = 100
    ) -> pd.DataFrame:
        """
        ...
        print("[INICIO] Iniciando extracción prudente con YouTube API v3...")
        ...
        if not df.empty:
            ...
            df["service_category"] = df.apply(
                lambda row: self.inferir_categoria(str(row["video_title"]) + " " + str(row["comment_text"])), axis=1
            )
            # Ordenar por fecha cronológica descendente
            df = df.sort_values(by="published_at", ascending=False).reset_index(drop=True)

        print("\n" + "=" * 55)
        print("[RESUMEN] Extracción en vivo finalizada exitosamente.")
        print(f"[RESUMEN] Comentarios únicos recolectados: {len(df)}")
        print(f"[RESUMEN] Videos procesados: {len(videos_procesados)}")
        print(
            f"[RESUMEN] Cuota consumida: {self.quota_used} de 10,000 unidades ({(self.quota_used / 10000) * 100:.2f}%)"
        )
        print("=" * 55 + "\n")
