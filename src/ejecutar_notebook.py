...
def generar_notebook_ejecutado():
    ...
    clean_sample_df = df_proc[
        [
            "comment_text",
            "clean_text",
            "service_category" if "service_category" in df_proc.columns else "topic_category",
        ]
    ].head(5)
        ...
        "cells": [
            {
                ...
                "source": [
                    ...
                    "3. Implementar un pipeline de **NLP Deep Learning** con Transformer preentrenado para español (BETO - `finiteautomata/beto-sentiment-analysis`) con mecanismos de autoatención bidireccional y cero fallback léxico.\n",
            ...
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": ["### 2. Ingesta y Exploración de Datos Crudos (YouTube Data API v3)"],
            },
            {
                "cell_type": "code",
                "execution_count": 2,
                ...
                "outputs": [
                    {
                        ...
                        "output_type": "stream",
                        "text": [
                            f"Total de registros crudos cargados: {len(df_raw)}\n",
                ...
                "source": [
                    "raw_path = '../data/raw/youtube_claro_raw.csv'\n",
                    "df_raw = pd.read_csv(raw_path)\n",
                    'print(f"Total de registros crudos cargados: {len(df_raw)}")\n',
                    'print(f"Columnas disponibles: {list(df_raw.columns)}\\n")\n',
                    'print("Primeros registros del dataset:")\n',
                    "df_raw[['video_title', 'author', 'comment_text', 'published_at', 'like_count']].head(5)",
                ],
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": ["### 3. Preprocesamiento Lingüístico de Textos en Español"],
            },
            {
                "cell_type": "code",
                "execution_count": 3,
            ...
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": ["### 4. Clasificación de Sentimiento con Transformer Preentrenado (BETO)"],
            },
            {
                "cell_type": "code",
                "execution_count": 4,
                ...
                "outputs": [
                    {
                        ...
                        "output_type": "stream",
                        "text": [
                            f"[NLP] Inferencia Deep Learning completada para {len(df_proc)} comentarios (Cero Fallback).\n",
                ...
                "source": [
                    "pipeline_sentimiento = SentimentTransformerPipeline(model_name='finiteautomata/beto-sentiment-analysis')\n",
                    "df_scored = pipeline_sentimiento.procesar_dataframe(df_clean, columna_texto='clean_text')\n",
                    'print(f"[NLP] Inferencia Deep Learning completada para {len(df_scored)} comentarios (Cero Fallback).")\n',
            ...
            {"cell_type": "markdown", "metadata": {}, "source": ["### 6. Visualización Interactiva con Plotly"]},
            {
                "cell_type": "code",
                "execution_count": 6,
                "metadata": {},
                ...
                "outputs": [
                    {
                        "data": {
                            ...
                            "text/plain": ["<Figure: Sentimiento de Marca por Categoría de Servicio (Claro RD)>"],
                        },
                        "metadata": {},
                        "output_type": "display_data",
                    }
