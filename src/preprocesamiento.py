...
def clasificar_topico_reglas(texto: str) -> str:
    ...
    t = texto.lower()
    if any(
        k in t for k in ["fibra", "internet", "wifi", "modem", "megas", "ping", "subida", "bajada", "router", "lento"]
    ):
        ...
        k in t for k in ["servicio", "atencion", "soporte", "call center", "tecnico", "espera", "oficina", "reporte"]
    ):
        return "Atención al Cliente y Soporte"
    elif any(
        k in t for k in ["factura", "cobro", "precio", "pago", "caro", "aumento", "recarga", "corte", "dinero", "pesos"]
    ):
        return "Facturación y Tarifas"
    elif any(
        k in t for k in ["promo", "oferta", "plan", "pospago", "prepago", "cambio", "equipo", "celular", "upgrade"]
    ):
