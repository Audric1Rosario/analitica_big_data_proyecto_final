...
def crear_documento_word(md_path: str = "INFORME_FINAL_CLARO.md", docx_out: str = "INFORME_FINAL_CLARO.docx"):
    ...
    while i < len(lineas):
        ...
        if line.startswith("## 6."):
            seccion_actual = line.replace("##", "").strip()
            h2 = doc.add_paragraph()
            h2.paragraph_format.space_before = Pt(14)
            h2.paragraph_format.space_after = Pt(6)
            h2.paragraph_format.keep_with_next = True
            run_h2 = h2.add_run(seccion_actual)
            run_h2.bold = True
        ...
        # Viñetas y Referencias APA 7
        elif line.startswith("* ") or line.startswith("- "):
            texto_bullet = line[2:].strip()
            if "6.20" in seccion_actual:
                # Formato APA 7: Párrafo con Sangría Francesa (Hanging Indent)
                p_ref = doc.add_paragraph()
                p_ref.paragraph_format.left_indent = Inches(0.5)
                p_ref.paragraph_format.first_line_indent = Inches(-0.5)
                p_ref.paragraph_format.space_after = Pt(4)
                p_ref.paragraph_format.line_spacing = 1.15
                # Parsear cursivas de títulos (*título*)
                partes = re.split(r"(\*.*?\*)", texto_bullet)
                for parte in partes:
                    if parte.startswith("*") and parte.endswith("*") and len(parte) > 2:
                        r = p_ref.add_run(parte[1:-1])
                        r.italic = True
                    else:
                        p_ref.add_run(parte)
                i += 1
        ...
        # Detección de Imágenes en Markdown: ![Alt](ruta)
        elif re.match(r"^!\[(.*?)\]\((.*?)\)$", line):
            img_match = re.match(r"^!\[(.*?)\]\((.*?)\)$", line)
            _alt_text = img_match.group(1)
        ...
        # Detección de Leyendas de Figuras: *Figura X: ...*
        elif line.startswith("*Figura ") or line.startswith("* Figura "):
            p_cap = doc.add_paragraph()
            p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
            p_cap.paragraph_format.space_before = Pt(2)
            p_cap.paragraph_format.space_after = Pt(10)
            run_cap = p_cap.add_run(line.replace("*", "").strip())
            run_cap.italic = True
            run_cap.font.size = Pt(9.5)
            run_cap.font.color.rgb = RGBColor(100, 116, 139)

        # Bloques de código o diagramas (ignorar contenido de ``` a ```)
        elif line.startswith("```"):
            i += 1
        ...
        # Párrafos normales
            ...
            # Omitir separadores horizontales
            if line.startswith("---"):
                i += 1
                continue
            p = doc.add_paragraph()
